...
exports.Ronn = function(text, version, manual, date) {
	...
	function toHTML(node) {
		...
		var fHtml = md.toHTMLTree(node);
		return md.renderJsonML(fHtml, {root:true, xhtml:true});
	}

	function toHTMLfragment(node) {
		var fHtml = md.toHTMLTree(node);
		return md.renderJsonML(fHtml, {root:false, xhtml:true});
	}
...
function prepareTree(pJml) {
	//console.log(sys.inspect(pJml, true, null));
	//return;
	// get references links (link_ref) from pJml root attributes.references
