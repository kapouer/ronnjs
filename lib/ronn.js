...
var md = require('./ext/markdown');
var sys = require('sys');

exports.roff = function(text) {
	html = md.toHTMLTree(text);
	//sys.puts(sys.inspect(html, false, null));
	return blockFilter("", html, {parent:null, previous:null, position:null});
};
...
function inlineFilter(out, node, context) {
	...
	switch(tag) {
		...
		case "a":
			var fStr = node[0];
			if (attributes['href'] == fStr || decodeURI(href) == "mailto:" + decodeURI(fStr)) {
				out += '\\fI';
				out = callInlineChildren(out, node, context);
				out += '\\fR';
			} else {
...
function comment(str, text) {
	return writeln(str, '.\\" ' + text);
}
...
function manDate() {
	var fDate = new Date();
	var fMonth = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"][fDate.getMonth()];
